...
<template>    
    ...
    <div class="hidden" id="addPoem">
      ...
      <input type="text" required name="title" v-model="title" placeholder="edit me"><br>
        ...
        <label>Poem's lang</label><br>
        <textarea required name="langue" v-model="langue" placeholder="edit me" rows="1" cols="50">Choose a lang
        </textarea> <br>
      ...
      <button v-on:click="fetchPoems()">List of poems</button><br>
        <br><text>Rechercher</text><input type="text" required name="title" v-model="mot" placeholder="edit me"><br>
        <button v-on:click="Filtrer()">Lancer</button><br>
    ...
    async createPoem(){
      var res;
        ...
        const { data, error }  =  await supabase
            .from('poems')
            .insert([
            { hidden: this.hidden, email:this.email, title: this.title, content: this.content, illustrationurl: res, langue: this.langue} ])
        if(error) throw(error)
    ...
    async fetchPoems(){
      //mange supabase access exceptions
        ...
        if(data.length>0){
            ...
            document.getElementById('poemlangue').value=data[0].langue
        }
    ...
    async Filtrer(){
      //mange supabase access exceptions
        ...
        if(data.length>0){
            ...
            document.getElementById('poemcontent').value=data[0].content
            document.getElementById('poemillustration').src=data[0].illustrationurl
            document.getElementById('poemillustration').value=data[0].langue
        }
        //store the indexof the currently displayed poem
        currentpoem=0;
      } catch (error) {
